...
class SistemaRH:
    ...
    def calc_decterceiro(self, salarioBruto, taxaINSS, taxaIRRF):
        ...
        while True:
            try:
                nmt = float(input('Tempo total de trabalho esse ano (em meses): '))
                if nmt <= 0:
                    raise
                else: 
                    break
                ...
                valorPrimario = (self.salarioBruto / 12) * nmto
                primeiraParcela = valorPrimario / 2

                valorSecundario = (self.salarioBruto / 12) * nmt
                segundaParcela = valorSecundario - (primeiraParcela + self.taxaINSS + self.taxaIRRF)
